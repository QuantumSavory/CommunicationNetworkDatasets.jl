...
service. It publishes separate semantic networks for `live=true` and `live=false`.
...
- Dataset license: `GNU GPL 2.0 (implied)`; see [LICENSE.md](LICENSE.md) for the scope caveat
...
The upstream `contrib_email` property is omitted from the published source-attribute JSON because
it is not required for network provenance; all 144 source geometries remain represented.
...
No source record is silently excluded.
